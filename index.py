...
def scrape_news(url):
    ...
    if response.status_code == 200:
        ...
        return [item.text.strip() for item in news_items if ("B-TECH" in item.text.upper() or "B.TECH" in item.text.upper() or "B. TECH" in item.text.upper()) and "RESULT" in item.text.upper()]
    else:
